...
class DRAM:
    ...
    def access(self, channel, rank, bank_group, bank, row, timestamp, operation):
        ...
        #self.previous_rank_state[(channel, rank)] = rank
        #self.previous_bg_state[(channel, rank, bank_group)] = bank_group
        #self.previous_bank_state[(channel, rank, bank_group, bank)] = bank
...
def model_contention(accesses, ini_file, cxl_ini_file):
    ...
    cxl_memory_parameters = load_memory_parameters(cxl_ini_file)
    global TOTAL_DRAM_VOTES
    global TOTAL_CXL_VOTES
    global TOTAL_STALL
    ...
    for i in range(0, len(accesses), 512):
        ...
        chunk = accesses.iloc[i:i + 512]

        if chunk.empty:
            continue

        last_dram_ts_in_chunk = -1.0
        last_dram_ets_in_chunk = -1.0
        last_cxl_ts_in_chunk = -1.0
        last_cxl_ets_in_chunk = -1.0

        for _, access in chunk.iterrows():
            physical_address = int(access['Physical Address'], 16)
            timestamp = access['Time Compressed Timestamp (ns)']
            operation = access['RW']
            ...
            effective_timestamp = timestamp + time_offset

            is_dram = (4294967296 <= physical_address <= 551903297535) or (1649267441664 <= physical_address <= 2199023255551)

            if is_dram:
                (channel, rank, bank_group, bank, row) = local_dram_obj.decode_address(physical_address)
                access_type, latency = local_dram_obj.access(
                    channel, rank, bank_group, bank, row, effective_timestamp, operation
                )
                last_dram_ts_in_chunk = timestamp
                last_dram_ets_in_chunk = effective_timestamp
            else: # CXL
                (channel, rank, bank_group, bank, row) = local_cxl_obj.decode_address(physical_address)
                access_type, latency = local_cxl_obj.access(
                    channel, rank, bank_group, bank, row, effective_timestamp, operation
                )
                last_cxl_ts_in_chunk = timestamp
            ...
            results['Time Compressed Timestamp (ns)'].append(timestamp) # Store original ts
            ...
            results['New Time Stamp (ns)'].append(effective_timestamp)
            results['Stall'].append(stall)
        
        time_offset = max(last_dram_ets_in_chunk, last_cxl_ets_in_chunk)
        if last_dram_ets_in_chunk > last_cxl_ets_in_chunk:
            TOTAL_DRAM_VOTES = TOTAL_DRAM_VOTES + 1
        else:
            TOTAL_CXL_VOTES = TOTAL_CXL_VOTES + 1
...
def main(output_folder, input_file):
    ...
    cxl_ini_file = 'cxl_memory_config.ini'
    ...
    with open(summary_file, "w") as summary:
        for app in apps:
            for run_number in run_numbers:
                ...
                avg_read_latency, avg_write_latency, avg_interarrival, last_timestamp, tot_rows, results = model_contention(accesses, ini_file, cxl_ini_file)
                # Prepare output file names
                results_file = os.path.join(output_folder, f"stall_2_results_{app}_{run_number}.csv")
                
                # Write results to CSV
                save_results(results, results_file)
                print("DRAM VOTES : ", TOTAL_DRAM_VOTES)
                print("CXL VOTES : ", TOTAL_CXL_VOTES)
                print("TOTAL STALL : ", TOTAL_STALL)
