    ...
    for index, access in accesses.iterrows():
        ...
        if len(dram_system.transaction_queues[channel]) < dram_system.transaction_queue_length:
            ...
            if len(dram_system.bank_cmd_queue[channel][rank][bank_group][bank]) < dram_system.cmd_queue_length:
                dram_system.bank_cmd_queue[channel][rank][bank_group][bank].append((physical_address, timestamp, operation, accepted, channel, rank, bank_group, bank, row))
            else:
                dram_system.transaction_queues[channel].append((physical_address, timestamp, operation, accepted, channel, rank, bank_group, bank, row))    
        else:
            accepted = False
                    ...
                    for f_bank in list(dram_system.bank_cmd_queue[f_channel][f_rank][f_bank_group].keys()):
                        ...
                        while dram_system.bank_cmd_queue[f_channel][f_rank][f_bank_group][f_bank]:
                            ...
                            if output_timestamp <= timestamp:
                                ...
                                results['Tque'].append(len(dram_system.bank_cmd_queue[q_channel][q_rank][q_bank_group][q_bank]))
                                results['CMD_que'].append(len(dram_system.transaction_queues[q_channel]))
    ...
    with open(summary_file, "w") as summary:
        for app in apps:
            for run_number in run_numbers:
                input_file = f'/data/{app}_run{run_number}_dram_input.csv'
...
if __name__ == "__main__":
    output_folder = "/data/output"  # Adjust this path as needed
